# -*- coding: utf-8 -*-
from tkinter import Tk,Frame,END,Button,PhotoImage,Label,Entry,filedialog,font,W,Scrollbar
import os
...
class KNN:
    ...
    k - knn算法的k
    '''
    def classify(self,inputPoint,dataSet,labels,k):
    ...
    '''
    def classnumCut1(self,fileName):
        fileStr = fileName.split('.')[0]
    ...
    def trainingDataSet(self):
        ...
        for i in range(m):
            ...
            hwLabels.append(self.classnumCut1(fileNameStr))#hwLabels记录了每个文件代表的数字分类
            trainingMat[i,:] = self.pretreatment('test/%s' % fileNameStr)#trainingMat的每一行保存了0/1代表的处理后的一张图片
        t2 = time.time()
    ...
    def train(self):
        self.hwLabels,self.trainingMat = self.trainingDataSet()    #构建训练集
    #测试函数
    def test(self,file):
        
        vectorUnderTest = self.pretreatment(file) #预处理图片        
        classifierResult = self.classify(vectorUnderTest, self.trainingMat, self.hwLabels, 3)#调用knn算法进行分类
        return classifierResult 
        
    '''
    def test(self):
        
        hwLabels,trainingMat = self.trainingDataSet()    #构建训练集
...
class Window:
    ...
    def open(self):
        dir =self.entry.get()
        testFileList = listdir(dir)
        
        length=len(testFileList)
        
        ft = font.Font(size=20, weight=font.BOLD)
        
        self.label_t.config(text="")
        
        for i in range(length):
            filename=dir+'/'+testFileList[i]
            myImage=PhotoImage(file=filename)
            l = Label(self.frmC,image=myImage)
            l.image=myImage
            l.grid(row=i,column=0,sticky=W)
            
            Label(self.frmC,text="这张图片是："+str(self.knn.test(filename)), font=ft).grid(row=i,column=1)
    ...
    def browse(self):
        default_dir = r"文件路径"
        file_path = filedialog.askdirectory(title=u'选择文件', initialdir=(os.path.expanduser(default_dir)))
    ...
    def start(self):
        ...
        Button(frmA,text="浏览",width=8,command=self.browse).grid(row=0,column=0)
        self.entry = Entry(frmA,width=40)
        self.entry.grid(row=0,column=1)
        Button(frmA,text="打开",width=8,command=self.open).grid(row=0,column=2)
        
        self.frmC = Frame()
        
        ft = font.Font(size=20, weight=font.BOLD)
        
        self.label_t=Label(self.frmC,text="请选择图片文件夹",fg='green',font=ft)
        self.label_t.grid()
        

        
        
        frmA.grid(row=0,column=0)
        #frmB.grid(row=1,column=0)
        self.frmC.grid(row=1,column=0, pady=10)
